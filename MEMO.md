...
micro-SIP — has a **median portfolio of Rs 862** and a **maximum of Rs
11,841**. Nobody clears the real minimum. We only got simulated loans at all by
assuming a Rs 2,000 threshold, ~12x under market, purely so the rest of this
analysis had a population to work with (`ASSUMPTIONS.md`).

The unit economics model derives the same wall independently: fixed
per-loan costs (servicing + acquisition) divided by the risk-adjusted fee
...
|---|---|---|
| Rs 1,800 (**today**) | -Rs 232 | ❌ |
| Rs 10,000 | -Rs 194 | ❌ |
| Rs 50,000 | -Rs 9 | ❌ |
| Rs 75,000 | +Rs 107 | ✅ |

Viability starts around a **Rs 52,000 average portfolio** — ~29x the Rs 1,800
average this scenario assumes today, and ~60x the book's actual median
portfolio (Rs 862). That's a multi-year maturity question, not a targeting or
disbursal-rate question.

## Who to target, when it's viable

The propensity model (logistic regression, AUC 0.870, 3.5x lift at the top 100
users) says clearly: **consistency dominates collateral**.
...
## The nudge works — this part is good news now, not later
...
4. **Every number above is built on named assumptions**, most consequentially
   a 1.5% origination fee and Rs 180/loan servicing cost — both `ASSUMED`, not
