...
## [1.0.1] - Documentation and presentation pass

### Added

- `LICENSE` (MIT).
- `docs/index.html`, a live findings page hosted on GitHub Pages, rebuilt
  from the pipeline's own output rather than hand-typed.
- Real chart images (`docs/images/`) committed as documentation assets:
  the propensity model's odds-ratio plot and the nudge validation's
  breakage-by-arm chart, both generated by the actual analysis code.
- CI status badge, release badge, test-count badge, and license badge in
  the README.
- Entity-relationship and pipeline-flow diagrams (Mermaid) in the README,
  showing the shared-database dependency on Cadence explicitly.
- A direct odds-ratio table and nudge results table in the README itself,
  rather than only asserting the headline numbers in prose.

### Changed

- Removed the phrase "the build is complete" from the README. It was an
  unqualified claim with nothing in the document to verify it against; the
  CI badge and the phase-by-phase table now let a reader check the claim
  instead of taking it on faith.
- Restructured the README end to end: project overview, live links,
  problem statement, key outcome, architecture, quickstart, pipeline
  workflow, core analysis with real figures and images, dashboard summary,
  key findings, repository layout, technology stack, and documentation
  index, in that order.
- Removed em dashes throughout every markdown document and the SQL comments
  that surface as Metabase card descriptions, replaced with plain
  punctuation for a more direct, less stylised tone.
