...
|---|---|---|---|
| 1 | LAMF funnel, last 12 weeks | bar | Applications -> eligible -> approved -> disbursed, one source for every drop-off. |
...
5,000 users / 373,387 transactions):

**Card 6 — the number that reframes the whole project.** Average disbursed
ticket is **Rs 1,218.99**, against a market-standard minimum of **Rs 25,000** —
...
None embedded, for the same reason Cadence's notes give: the dashboard is
verified programmatically (every card's query is run directly against the live
database above, with the actual row counts and values quoted), and that
verification rebuilds on any machine rather than describing one instant of one
instance. Run `make dashboard` and look at the real thing.
