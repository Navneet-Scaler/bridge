"""Shared chart styling, so every figure in Bridge reads as one system.

Same tokens and the same validated palette as Cadence's ``src/analysis/viz.py``,
so a reader moving between the two repos sees one visual system rather than two.
...
class Theme:
    ...
    status_critical: str
    deemphasis: str
    # Diverging pair for effects measured against a baseline (odds ratio 1.0,
    # hazard ratio 1.0). Blue and red read as opposite; a gray midpoint reads as
    # "no effect". Two cool hues would fail that — the midpoint has to read as
    # nothing at all.
    diverging_low: str
    diverging_high: str
...
def new_axes(figsize: tuple[float, float] = (8.4, 4.8)) -> tuple[plt.Figure, plt.Axes]:
    """Create a figure/axes pair. Call after :func:`apply_style`."""
