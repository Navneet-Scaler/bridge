"""Database connectivity for Bridge.
...
Bridge connects to **Cadence's database**, not one of its own. Bridge's tables
are a schema extension: they carry foreign keys into Cadence's ``users`` table,
and the propensity model reads Cadence's ``v_user_consistency`` view directly as
a feature source. Pointing this at a fresh, empty database will produce foreign
key violations at load time rather than a silently different result, which is the
intended failure mode — a missing dependency should be loud.
"""
